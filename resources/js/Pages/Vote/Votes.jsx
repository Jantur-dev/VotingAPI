...
const Vote = ({status, msg}) => {
  const [selectedCandidate, setSelectedCandidate] = useState(null);

  const id_user = localStorage.getItem('_login');
                ...
                <div className={`relative flex-1 flex-col justify-around ${styles.flexCenter} xxs:min-w-[292px] min-w-[120px] mx-[.5rem] my-1 px-6 py-[1.5rem] rounded-lg border-b-4 shadow-pit border-blue-900 bg-[#FFFFFF]`}>
                  ...
                  <div className="relative">
                    <form>
                      ...
                      />
                      <label htmlFor={`${client.id}`} className="absolute bottom-0 left-1/2 transform -translate-x-1/2 -mb-12 cursor-pointer">
                        <span
                          className={`inline-block w-10 h-8 border-4 rounded-full transition-all duration-300 ${selectedCandidate === client.id ? "bg-blue-900 border-blue-900" : "bg-white border-primary"}`}
                          style={{
