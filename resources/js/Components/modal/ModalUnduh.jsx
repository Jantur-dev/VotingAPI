...
import { Checklist_Hijau, Not_vote } from "@/assets";

const ModalUnduh = ({ done }) => {
    ...
    <div className="flex justify-center items-center mt-[4rem]">
      ...
      <Modal isOpen={modalIsOpen} onRequestClose={closeModal} contentLabel="Example Modal" style={customModalStyles}>
        <div className="flex flex-col items-center">
          <h1 className={`text-[25px] uppercase font-bold mb-4 ${done ? "text-[#05A660]" : "text-[#E53535]"} py-[1rem]`}>{done ? "Berhasil Diunduh" : "Gagal mengunduh Data"}</h1>
          <img src={done ? Checklist_Hijau : Not_vote} alt="Image" className="w-24 h-24 rounded-full mb-4" />
          <p className="text-center mb-4 text-[16px] text-primary font-semibold py-[1rem]">{done ? "Data Suara Berhasil Diunduh" : "Anda Belum Dapat Menlihat Data Suara Sekarang"}</p>
